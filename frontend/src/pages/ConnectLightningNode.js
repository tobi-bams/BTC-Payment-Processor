...
const ConnectLightningPage = () => {
  ...
  function submitHandler(event) {
    ...
    fetch(
      "http://localhost:5000/wallet/create-lightning/" +
        authCtx.currentUser.data.store.id,
      {
        method: "POST",
        body: JSON.stringify({
      ...
      .then((data) => {
        setTimeout(async () => {
          ...
          Swal.fire({
            title: "Wallet",
            text: "Lightning Watch only wallet Created Successfully",
            icon: "success",
  ...
  return (
    <>
      <div className="mb-10">
        ...
        <p>
          Before starting, please convert your node's admin.macaroon file and
          tls.cert file to hex format and paste the strings in their respective
          fields below.{" "}
        </p>
        ...
        <form className="w-full mx-auto text-center" onSubmit={submitHandler}>
          ...
          <FormGroup>
            <textarea
              ...
              rows="4"
              id="macaroon"
              ref={macaroonInputRef}
              placeholder="admin.macaroon"
              required
            />
          </FormGroup>
          <FormGroup>
